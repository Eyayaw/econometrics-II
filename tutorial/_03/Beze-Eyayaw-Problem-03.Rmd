...
& = \frac{Cov(\beta_0 + \beta_1 \bar x_i,\bar x_i) + Cov(a_i, \bar x_i) + \overbrace{Cov(\bar u_i, \bar x_i)}^{= \;0 \;by \;orthogonality}}{Var(x)} \\
& = \frac{Cov(\beta_0 + \beta_1 \bar x_i,\bar x_i) + \sigma_{xa} }{Var(x)}\\
& = \frac{\beta_0 E(\bar x_i) + \beta_1 E(\bar x_i^2) - E(\bar x_i)\bigl[\beta_0 + \beta_1E(\bar x_i)\bigr] + \sigma_{xa}}{Var(\bar x_i)}\\
& = \frac{\beta_1 \left[E(\bar x_i^2) - [E(\bar x_i)]^2\right] +\sigma_{xa}}{Var(\bar x_i)}\\
...
The assumption that the $x_{it}$'s are uncorrelated overtime is not necessarily reasonable. Because $x_{it}$'s could be serially correlated overtime for some i's or for some $x_i$'s such as income, education, knowledge etc. Income of individuals is likely to be correlated overtime. 
...
However, if the treatment is correlated with an unobserved individual effect, $\alpha_i$, we may try to kill it (i.e, the fixed effect) by differencing,
...
This may not result in consistent estimator for the treatment effect. The problem here is that the differenced residual, $\Delta u_{it}$, is necessarily correlated with the lagged dependent variable, $\Delta y_{i{t-1}}$, because both are a function of $u_{i{t-1}}$. Consequently, the OLS estimates of the first differenced equation (\ref{fd-ldv}) are not consistent for the parameters in equation (\ref{fe-ldv}) [@angrist-pischke2009, pp. 182-185].

Since we have more than three time periods, one solution to the above problem is to use $y_{i{t-2}}$ as an instrument for $\Delta y_{i{t-2}}$ in (\ref{fd-ldv}). But this requires that $y_{i{t-2}}$ be uncorrelated with the differenced residuals, $\Delta u_{it}$. However, if $u_{it}$ is serially correlated, there may be no consistent estimator for (\ref{fd-ldv}).
...
Mistakenly using fixed effects or lagged dependent variable model to estimate the causal effect, while the above pooled OLS set up is correct, may result in either over or underestimating the estimates of the treatment effect.
...
The differenced residual, $\Delta u_{it}$, could be correlated with the lagged differenced residual $u_{i{t-1}}$. So in our estimation we should account for this serial correlation in calculating standard errors and test statistics.
